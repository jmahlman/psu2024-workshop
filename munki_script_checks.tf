...
resource "zentral_munki_script_check" "mcs-macos-os_hibernate_mode_apple_silicon_enable" {
  name = "[mSCP] - macOS - Enable Hibernate Mode (Apple Silicon)"
  description = trimspace(<<EODESC
...
resource "zentral_munki_script_check" "mcs-macos-os_hibernate_mode_destroyfvkeyonstandby_enable" {
  name = "[mSCP] - macOS - Enable DestroyFVKeyOnStandby on Hibernate"
  description = trimspace(<<EODESC
...
resource "zentral_munki_script_check" "mcs-macos-os_hibernate_mode_intel_enable" {
  name = "[mSCP] - macOS - Enable Hibernate Mode (Intel)"
  description = trimspace(<<EODESC
Hibernate mode _MUST_ be enabled.
...
[source,text]
----
By using this computer, you agree to the Open Gaming License Terms. Make sure you have your dice ready!
----
EODESC
  )
  type = "ZSH_INT"
...
resource "zentral_munki_script_check" "mcs-macos-os_world_writable_library_folder_configure" {
  name = "[mSCP] - macOS - Ensure No World Writable Files Exist in the Library Folder"
  description = trimspace(<<EODESC
...
resource "zentral_munki_script_check" "mcs-passwordpolicy-pwpolicy_account_lockout_enforce" {
  name = "[mSCP] - Password Policy - Limit Consecutive Failed Login Attempts to 10"
  description = trimspace(<<EODESC
The macOS _MUST_ be configured to limit the number of failed login attempts to a maximum of 10. When the maximum number of failed attempts is reached, the account _MUST_ be locked for a period of time after.
...
EODESC
  ...
  type = "ZSH_STR"
  source = trimspace(<<EOSRC
/usr/bin/pwpolicy -getaccountpolicies 2> /dev/null | /usr/bin/tail +2 | /usr/bin/xmllint --xpath '//dict/key[text()="policyAttributeMaximumFailedAuthentications"]/following-sibling::integer[1]/text()' - | /usr/bin/awk '{ if ($1 <= 10) {print "yes"} else {print "no"}}'
EOSRC
  )
  expected_result = "yes"
  arch_amd64      = true
...
resource "zentral_munki_script_check" "mcs-systemsettings-system_settings_hot_corners_secure" {
  name = "[mSCP] - System Settings - Secure Hot Corners"
  description = trimspace(<<EODESC
...
resource "zentral_munki_script_check" "mcs-systemsettings-system_settings_location_services_enable" {
  name = "[mSCP] - System Settings - Enable Location Services"
  description = trimspace(<<EODESC
...
resource "zentral_munki_script_check" "mcs-systemsettings-system_settings_location_services_menu_enforce" {
  name = "[mSCP] - System Settings - Ensure Location Services Is In the Menu Bar"
  description = trimspace(<<EODESC
...
EOSRC
  ...
  expected_result = base64encode("By using this computer, you agree to the Open Gaming License Terms. \n")
  arch_amd64      = true
  arch_arm64      = true
  min_os_version  = "14"
  max_os_version  = "15"
...
resource "zentral_munki_script_check" "mcs-systemsettings-system_settings_time_machine_auto_backup_enable" {
  name = "[mSCP] - System Settings - Configure Time Machine for Automatic Backups"
  description = trimspace(<<EODESC
